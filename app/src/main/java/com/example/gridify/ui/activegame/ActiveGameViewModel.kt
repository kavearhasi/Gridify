...
class ActiveGameViewModel {
    ...
    fun initializeBoardState(puzzle: SudokuPuzzle, isComplete: Boolean) {
        puzzle.graph.forEach {
            val node = it.value[0]
            boardState[it.key] = SudokuTile(node.x, node.y, node.color, node.readOnly, false)
        }
        val contentState:ActiveGameScreenStat
        if(isComplete){
...
data class SudokuTile(
    val x: Int, val y: Int, var value: Int, val readOnly: Boolean, var hasFocus: Boolean
)